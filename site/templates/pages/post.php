<div class="flex flex-col justify-end relative min-h-96 text-spark-pink-100 bg-center bg-cover bg-no-repeat before:content-['\00a0'] before:bg-gradient-to-t before:from-courageous-plum-800 before:absolute before:size-full" <?php if ($image = $Post->getImageUrl()) : ?>style="background-image: url('<?= $image ?>');"<?php endif; ?>>
...
<div class="bg-[#FAF3F9] grow min-h-full px-28 text-black pt-16 tracking-tight z-10">
	<div class="flex gap-16 -mt-32 bg-[#FAF3F9] p-10 rounded-t-lg">
		...
		<section class="w-3/5">
			...
			<p class="mt-6 leading-loose"><?= $Post->getFullDescription() ?></p>
		</section>
		...
		<aside class="w-2/5 h-fit mt-2 flex flex-col divide-y grow-0 overflow-hidden rounded-md bg-white shadow">
			...
			<div class="flex justify-between px-6 py-4">
				<h4>Location</h4>
				<p class="text-right">
					<?php //$Post['partner']['address']['street'] ?><br />
					<?php //$Post['partner']['address']['city'] . ', ' . $Post['partner']['address']['state'] . ' ' . $Post['partner']['address']['zip'] ?>
				</p>
			</div>
			<?php if ($Post->getSchedule()) : ?>
				<div class="flex justify-between px-6 py-4">
					<h4>Schedule</h4>
					<dl>
						<dt>Monday</dt>
